...
- **Multi-tenancy desde o início**: nenhum endpoint tem `professional_id` fixo no código. O identificador do profissional (por enquanto, algo simples como um slug — sem exigir login completo ainda) é sempre um parâmetro, tanto no upload de documentos quanto no chat. Cada profissional tem sua própria coleção no ChromaDB, isolada das demais. Isso permite demonstrar dois ou mais profissionais funcionando lado a lado, com tom de voz e base de conhecimento diferentes, provando o conceito de plataforma sem precisar de autenticação completa, cobrança ou onboarding self-service do WhatsApp ainda.
...
└── app/
    ...
    ├── models.py          # tabelas: Professional, Document, Conversation, Message, SafetyAlert
    ├── schemas.py         # formatos de entrada/saída da API
    └── routers/
        └── chat.py        # canal simulado (POST /chat/simulate) — resposta ainda é placeholder
```
...
1. **Finalizar RAG + resposta real, já multi-tenant** — endpoint de upload de documentos e endpoint de chat recebem um identificador de profissional (slug simples, sem login completo ainda); coleção própria no ChromaDB por profissional; resposta real via LLM no lugar do placeholder.
2. **Desenhar o protocolo de segurança com a profissional** (em paralelo ao passo 1) — o que ela considera sinal de risco, quão conservador o sistema deve ser, como ela quer ser notificada.
...
- Painel/dashboard para o profissional gerenciar seus próprios documentos e configurações
